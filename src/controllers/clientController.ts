 ...
 * @returns Promesse résolue avec la liste des Client, ou une liste vide en cas d'erreur.
 */
...
export type NewClientData = Omit<Client, 'id_client' | 'date_inscription' | 'commandes'> & { password: string };

/**
 * Crée un nouveau client via l'API (POST).
 * @param clientData Les données du nouveau client, y compris un mot de passe.
 * @returns Promesse résolue avec l'objet Client créé (avec son nouvel id_client), ou null en cas d'échec.
 */
export async function createClient(clientData: NewClientData): Promise<Client | null> {
    const url = `${API_BASE_URL}/clients`;
    try {
        const response = await fetch(url, {
            method: 'POST',
 ...
 * @param id_client L'ID du client à mettre à jour.
 * @param clientData Les données partielles du client à mettre à jour.
 * @returns Promesse résolue avec l'objet Client mis à jour, ou null en cas d'échec.
 */
export async function updateClient(id_client: number, clientData: UpdateClientData): Promise<Client | null> {
    const url = `${API_BASE_URL}/clients/${id_client}`;
    try {
        const response = await fetch(url, {
            method: 'PUT',
            headers: {
        ...
        console.error(`Erreur réseau ou API lors de la mise à jour du client ${id_client}:`, error);
        return null;
    }
}
 ...
 */
export async function deleteClient(id_client: number): Promise<boolean> {
    const url = `${API_BASE_URL}/clients/${id_client}`;
    try {
        const response = await fetch(url, {
            method: 'DELETE',
        ...
        console.error(`Erreur réseau ou API lors de la suppression du client ${id_client}:`, error);
        return false;
    }
}